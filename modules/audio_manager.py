#Objetivo: Gestionar la comunicación con la API AssemblyAI, desde la 
#grabación de audio hasta la obtención del texto transcrito.

#Para acceder a vatiables de entorno de forma segura
import os
...
#Si no usa el valor por defecto "TU_API_KEY_AQUI"
...
#Frecuencia de muestro estándar para reconocimiento de voz
#16kHz es el estándar en industria de ASR(Automatic Speech Recognition)
SAMPLERATE = 16000

#Duración de la grabación en segundos
DURATION_SECONDS = 5

#Nombre temporal del archivo de audio (se borra)
...
def grabar_audio():
    ...
    Retorna: str: Ruta del archivo WAV grabado
    Proceso:
    ...
    audio = sd.rec(
        int(SAMPLERATE * DURATION_SECONDS),     #Número de muestras (5segundos)
...
def subir_audio(filepath):
    ...
    Proceso:
        ...
        3. Verifica código de repuesta HTTP 200.
    ...
    if response.status_code == 200:
        #Extrae la URL del archivo subido JSON de respuesta
...
def solicitar_transcripcion(audio_url):
    ...
    json_data = {
        ...
        "language_code": "es"       #Configura el idioma en español
    }
    ...
    if response.status_code == 200:
        #Extrar el ID único de la trancripción
...
def obtener_resultado_transcripcion(transcript_id):
    """
    Realiza polling (consultas periódicas) hasta quqe la transcripción esté completa o haya un error.
    Args:
        transcript:id (str): ID único de transcripción
    Re    
    Lanza:
